...
local function IsCatForm()
    if type(GetShapeshiftFormID) == "function" then
        ...
        if formID then
            -- Retail/current API path
            -- Cat Form is exposed through shapeshift form IDs.
            -- If your client returns 2 for cat, this covers it.
            if formID == 2 then
                return true
            end
        end
...
local function ShouldShowFrame(cfg)
...
function D:EnsureFrame()
    if self.frame then return self.frame end

    local f = CreateFrame("Frame", ADDON .. "CooldownManagerDruidComboBarFrame", UIParent)
...
function D:LayoutSegments(maxPoints)
    local cfg = P:GetCfg()
    local d = P.DEFAULT_CFG
    local f = self:EnsureFrame()
    local comboCfg = cfg.combo or d.combo
...
function D:UpdateSegments(curPoints, maxPoints)
    local f = self:EnsureFrame()
...
function D:Update()
    ...
    if not P:IsDruid() or not IsCatForm() then
        f:Hide()
        return
    end

    if cfg.enabled == false or not ShouldShowFrame(cfg) then
        f:Hide()
        return
    end
